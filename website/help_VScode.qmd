...
Now, each time you will need to open a VS session on the server, you will need to run this .bat or .sh file. But the first time, you will run this script, the terminal window will ask you to authorize the tunnel through Github (or Microsoft) and to give it a name. 

Name the tunnel as you prefer (for example: vscode_cluster_tunnel). Follow the instructions (make sure you are connected to Github in your web browser, open the link and copy paste the authorization code)
