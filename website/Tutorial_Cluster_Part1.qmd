...
If you wish instead to use python [venv](https://docs.python.org/3/library/venv.html) to manage your virtual environment,
here is an example that make use of the default Python interpreter already installed on the cluster (currently,
version 3.11). To create a venv named `crnlvenv` and load it in a Jupyter kernel just like
done above with miniconda, type the following commands:

```{.bash}
### Create ~/.virtualenvs if it does not alread exist 
mkdir -p ~/.virtualenvs
### Create a venv named crnlvenv
python3 -m venv .virtualenvs/crnlvenv
### Activate the new venv
source ~/.virtualenvs/crnlvenv/bin/activate
### Install some packages in crnlvenv
pip install --upgrade pip
pip install numpy submitit
pip install memory_profiler -U
### ipykernel creates a jupyter kernel from current venv
pip install ipykernel
python -m ipykernel install --user --name 'crnlvenv'
```
